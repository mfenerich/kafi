...
class Test(unittest.TestCase):
    def setUp(self):
        ...
        self.protobuf_key_schema_str = (
            "message SnackKey { required string key = 1; }"
        )
        ...
        #
        self.key_schema_str_list = [
            self.avro_key_schema_str,
            self.protobuf_key_schema_str,
            self.jsonschema_key_schema_str,
        ]
        self.value_schema_str_list = [
            self.avro_value_schema_str,
            self.protobuf_value_schema_str,
            self.jsonschema_value_schema_str,
        ]
        #
        self.storage_str_topic_str_list_dict = {
            "Cluster": [],
            "RestProxy": [],
            "AzureBlob": [],
            "Local": [],
        ...
        self.storage_str_group_str_list_dict = {
            "Cluster": [],
            "RestProxy": [],
            "AzureBlob": [],
            "Local": [],
    ...
    def create_test_topic_name(self, storage_obj):
        ...
        while True:
            ...
            if (
                topic_str
                not in self.storage_str_topic_str_list_dict[storage_str]
            ):
                self.storage_str_topic_str_list_dict[storage_str].append(
                    topic_str
                )
                break
        #
    ...
    def create_test_group_name(self, storage_obj):
        while True:
            ...
            if (
                group_str
                not in self.storage_str_group_str_list_dict[storage_str]
            ):
                self.storage_str_group_str_list_dict[storage_str].append(
                    group_str
                )
                break
        #
...
def test_cp(test_obj, storage1, storage2):
    ...
    w.produce(
        ...
        headers=test_obj.headers_str_bytes_tuple_list,
    )
    w.close()
    ...
    if storage1.__class__.__name__ == "RestProxy":
        # Need to use the native Kafka API here since the RestProxy V2 consumer
        # does not support timestamps.
        c = Cluster("local")
        (consume_n_int1, written_n_int1) = c.cp(
            topic_str1,
            storage2,
            topic_str2,
            group=group_str1,
            source_key_type="bytes",
            source_value_type="bytes",
            target_key_type="bytes",
            target_value_type="bytes",
            produce_batch_size=produce_batch_size_int1,
            n=3 * 3,
            keep_timestamps=True,
            keep_partitions=True,
        )
    else:
        (consume_n_int1, written_n_int1) = storage1.cp(
            topic_str1,
            storage2,
            topic_str2,
            group=group_str1,
    ...
    if storage2.__class__.__name__ == "RestProxy":
        ...
        # does not support timestamps.
        c = Cluster("local")
        (consume_n_int2, written_n_int2) = c.cp(
            topic_str2,
            storage1,
            topic_str3,
            group=group_str2,
            ...
            target_key_type="bytes",
            target_value_type="bytes",
            produce_batch_size=produce_batch_size_int2,
            n=3 * 3,
            keep_timestamps=True,
            keep_partitions=True,
        )
    else:
        (consume_n_int2, written_n_int2) = storage2.cp(
            topic_str2,
            storage1,
            topic_str3,
            group=group_str2,
    ...
    test_obj.assertEqual(
        list(storage1_topic1_watermarks_dict.values()),
        list(storage2_topic2_watermarks_dict.values()),
    )
    test_obj.assertEqual(
        list(storage2_topic2_watermarks_dict.values()),
        list(storage1_topic3_watermarks_dict.values()),
    )
    #
    ...
    if storage1.__class__.__name__ == "RestProxy":
        ...
        # does not support timestamps.
        c = Cluster("local")
        message_dict_list1 = c.cat(
            topic_str1, group=group_str3, type=type_str, n=3 * 3
        )
    else:
        message_dict_list1 = storage1.cat(
            topic_str1, group=group_str3, type=type_str, n=3 * 3
        )
    #
    ...
    if storage1.__class__.__name__ == "RestProxy":
        ...
        # does not support timestamps.
        c = Cluster("local")
        message_dict_list2 = c.cat(
            topic_str3, group=group_str4, type=type_str, n=3 * 3
        )
    else:
        message_dict_list2 = storage1.cat(
            topic_str3, group=group_str4, type=type_str, n=3 * 3
        )
    #
    test_obj.assertEqual(3 * 3, len(message_dict_list2))
    #
    storage1_topic1_timestamp_set = set(
        [message_dict["timestamp"] for message_dict in message_dict_list1]
    )
    storage1_topic3_timestamp_set = set(
        [message_dict["timestamp"] for message_dict in message_dict_list2]
    )
    test_obj.assertEqual(
        storage1_topic1_timestamp_set, storage1_topic3_timestamp_set
    )

    # Copy topic3 on storage1 to topic5 on storage2 as json and do a tiny
    # mapping.
    ...
    (consume_n_int3, written_n_int3) = storage1.cp(
        topic_str3,
        storage2,
        topic_str5,
        group=group_str5,
        ...
        target_type="json",
        produce_batch_size=produce_batch_size_int3,
    ...
    message_dict_list3 = storage2.cat(
        topic_str5, group=group_str6, type="json", n=3 * 3
    )
    ...
    if (
        storage1.__class__.__name__ != "RestProxy"
        and storage2.__class__.__name__ != "RestProxy"
    ):
        for i in range(3):
            j0 = next(
                j
                for j, message_dict in enumerate(message_dict_list3)
                if message_dict["value"]["name"]
                == snack_dict_list[3 * i]["name"]
            )
            j1 = next(
                j
                for j, message_dict in enumerate(message_dict_list3)
                if message_dict["value"]["name"]
                == snack_dict_list[3 * i + 1]["name"]
            )
            j2 = next(
                j
                for j, message_dict in enumerate(message_dict_list3)
                if message_dict["value"]["name"]
                == snack_dict_list[3 * i + 2]["name"]
            )
            #
            test_obj.assertEqual(
                message_dict_list3[j0]["partition"],
                message_dict_list3[j1]["partition"],
            )
            test_obj.assertEqual(
                message_dict_list3[j1]["partition"],
                message_dict_list3[j2]["partition"],
            )
            #
            test_obj.assertLess(
                message_dict_list3[j0]["offset"],
                message_dict_list3[j1]["offset"],
            )
            test_obj.assertLess(
                message_dict_list3[j1]["offset"],
                message_dict_list3[j2]["offset"],
            )
...
def test_from_to_file(test_obj, storage1, storage2):
    if storage2.__class__.__name__ in ["AzureBlob", "Local", "S3"]:
        ...
        n_int1 = storage1.topic_to_file(
            topic_str1, storage2, file_str, group=group_str1, type="json"
        )
        ...
        message_dict_list = storage1.cat(
        ...
        #
        storage2.admin.delete_file(
            storage2.admin.get_abs_path_str(f"files/{file_str}")
        )
