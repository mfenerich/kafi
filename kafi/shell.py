...
class Shell(Functional):
    ...
    def tail(self, topic, n=10, **kwargs):
        ...
        offsets_dict = {
            partition_int: -n_int for partition_int in range(partitions_int)
        }
    ...
    def wc(self, topic, **kwargs):
        def foldl_function(acc, message_dict):
            ...
            #
            acc_num_words_int = (
                acc[0] + num_words_key_int + num_words_value_int
            )
            acc_num_bytes_int = (
                acc[1] + num_bytes_key_int + num_bytes_value_int
            )
            return (acc_num_words_int, acc_num_bytes_int)

        #
        ((acc_num_words_int, acc_num_bytes_int), num_messages_int) = (
            self.foldl(topic, foldl_function, (0, 0), **kwargs)
        )
    ...
    def diff_fun(
        self, topic1, storage2, topic2, diff_function, n=ALL_MESSAGES, **kwargs
    ):
        def zip_foldl_function(acc, message_dict1, message_dict2):
            if diff_function(message_dict1, message_dict2):
                ...
                if self.verbose() > 0:
                    ...
                    print(
                        f"Found differing messages on 1) partition {partition_int1}, offset {offset_int1} and 2) partition {partition_int2}, offset {offset_int2}."
                    )
                #
            return acc

        #
        return self.zip_foldl(
            topic1, storage2, topic2, zip_foldl_function, [], n=n, **kwargs
        )

    def diff(self, topic1, storage2, topic2, n=ALL_MESSAGES, **kwargs):
        def diff_function(message_dict1, message_dict2):
            return (
                message_dict1["key"] != message_dict2["key"]
                or message_dict1["value"] != message_dict2["value"]
            )

        return self.diff_fun(
            topic1, storage2, topic2, diff_function, n=n, **kwargs
        )
    ...
    def grep_fun(
        ...
        **kwargs,
        ...
        def foldl_function(acc, message_dict):
            ...
            if match_function(message_dict):
                if self.verbose() > 0:
                    ...
                    print(
                        f"Found matching message on partition {partition_int}, offset {offset_int}."
                    )
                #
        ...
        matches_int = matches
        #
        ((matching_message_dict_list, _), message_counter_int) = self.foldl(
            topic, foldl_function, ([], 0), n=n, **kwargs
        )
        #
        return (
            matching_message_dict_list,
            len(matching_message_dict_list),
            message_counter_int,
        )
    ...
    def grep(
        ...
        **kwargs,
        ...
        def match_function(message_dict):
            ...
            value_str = str(message_dict["value"])
            return (
                pattern.match(key_str) is not None
                or pattern.match(value_str) is not None
            )

        #
        return self.grep_fun(
            topic, match_function, n=n, results=results, **kwargs
        )
