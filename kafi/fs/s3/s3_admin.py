...
class S3Admin(FSAdmin):
    def __init__(self, s3_obj):
        ...
        #
        self.minio = Minio(
            s3_obj.s3_config_dict["endpoint"],
            access_key=s3_obj.s3_config_dict["access.key"],
            secret_key=s3_obj.s3_config_dict["secret.key"],
            secure=False,
        )
    ...
    def list_dirs(self, abs_path_dir_str):
        object_generator = self.minio.list_objects(
            self.storage_obj.bucket_name(),
            prefix=abs_path_dir_str,
            recursive=True,
        )
        rel_dir_str_set = set(
            [
                os.path.basename(os.path.dirname(object.object_name))
    ...
    def list_files(self, abs_path_dir_str):
        ...
        rel_file_str_set = set(
            [
                os.path.relpath(object.object_name, abs_path_dir_str)
                for object in object_generator
            ]
        )
        #
    ...
    def delete_file(self, abs_path_file_str):
        self.minio.remove_object(
            self.storage_obj.bucket_name(), abs_path_file_str
        )
    ...
    def read_bytes(self, abs_path_file_str):
        ...
        self.minio = Minio(
            self.storage_obj.s3_config_dict["endpoint"],
            access_key=self.storage_obj.s3_config_dict["access.key"],
            secret_key=self.storage_obj.s3_config_dict["secret.key"],
            secure=False,
        )
        #
        response = self.minio.get_object(
