...
class AzureBlobAdmin(FSAdmin):
    def __init__(self, azureblob_obj):
        ...
        blobServiceClient = BlobServiceClient.from_connection_string(
            azureblob_obj.azure_blob_config_dict["connection.string"]
        )
        self.containerClient = blobServiceClient.get_container_client(
            azureblob_obj.container_name()
        )
    ...
    def list_dirs(self, abs_path_dir_str):
        blob_str_itemPaged = self.containerClient.list_blob_names(
            name_starts_with=abs_path_dir_str
        )
        rel_dir_str_set = set(
            [
                os.path.basename(os.path.dirname(blob_str))
                for blob_str in blob_str_itemPaged
            ]
        )
        #
    ...
    def list_files(self, abs_path_dir_str):
        ...
        rel_file_str_set = set(
            [
                os.path.relpath(blob_str, abs_path_dir_str)
                for blob_str in blob_str_itemPaged
            ]
        )
        #
    ...
    def exists_file(self, abs_path_file_str):
        ...
        blobClient = BlobClient.from_connection_string(
            self.storage_obj.azure_blob_config_dict["connection.string"],
            container_name=self.storage_obj.azure_blob_config_dict[
                "container.name"
            ],
            blob_name=abs_path_file_str,
        )
        #
    ...
    def read_str(self, abs_path_file_str):
        ...
        blobClient = BlobClient.from_connection_string(
            conn_str=self.storage_obj.azure_blob_config_dict[
                "connection.string"
            ],
            container_name=self.storage_obj.azure_blob_config_dict[
                "container.name"
