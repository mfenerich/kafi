...
class StorageConsumer(Deserializer):
    def __init__(self, storage_obj, *topics, **kwargs):
        ...
        self.topic_str_start_offsets_dict_dict = (
            self.get_start_offsets_from_kwargs(self.topic_str_list, **kwargs)
        )
        #
        ...
        ) = self.get_key_value_types_from_kwargs(self.topic_str_list, **kwargs)
        #
        # Get group.
        ...
        self.consumer_config_dict["auto.offset.reset"] = (
            storage_obj.auto_offset_reset()
        )
        if "config" in kwargs:
        ...
        self.enable_auto_commit_bool = (
            kwargs["enable.auto.commit"]
            if "enable.auto.commit" in kwargs
            else storage_obj.enable_auto_commit()
        )
        #
    ...
    def get_start_offsets_from_kwargs(self, topic_str_list, **kwargs):
        ...
        if topic_str_offsets_dict_dict is not None:
            offset_int_list = sum(
                {
                    topic_str: list(offsets_dict.values())
                    for topic_str, offsets_dict in topic_str_offsets_dict_dict.items()
                }.values(),
                [],
            )
            if any(offset_int < 0 for offset_int in offset_int_list):
                topic_str_partition_int_size_int_dict_dict = (
                    self.storage_obj.ls(topic_str_list, partitions=True)
                )
                topic_str_offsets_dict_dict = {
                    topic_str: {
                        partition_int: (
    ...
    def get_key_value_types_from_kwargs(self, topic_str_list, **kwargs):
        (key_type_str, value_type_str) = (
            self.storage_obj.get_key_value_type_tuple(**kwargs)
        )
        #
        topic_str_key_type_str_dict = {
            topic_str: key_type_str for topic_str in topic_str_list
        }
        topic_str_value_type_str_dict = {
            topic_str: value_type_str for topic_str in topic_str_list
        }
        #
