...
class ClusterAdmin(KafkaAdmin):
    ...
    def acls(
        ...
        aclBinding_list = self.adminClient.describe_acls(
            aclBindingFilter
        ).result()
        #
        return [
            aclBinding_to_dict(aclBinding) for aclBinding in aclBinding_list
        ]
    ...
    def delete_acl(
        ...
        aclBinding_list = self.adminClient.delete_acls([aclBindingFilter])[
            aclBindingFilter
        ].result()
        #
        return [
    ...
    def brokers(self, pattern=None):
        ...
        if pattern_int_or_str_list == [None]:
            ...
            pattern_str_list = [
                str(pattern_int_or_str)
                for pattern_int_or_str in pattern_int_or_str_list
            ]
        #
        broker_dict = {
            broker_int: brokerMetadata.host + ":" + str(brokerMetadata.port)
            for broker_int, brokerMetadata in self.adminClient.list_topics().brokers.items()
            if any(
                fnmatch(str(broker_int), pattern_str)
    ...
    def broker_config(self, pattern=None, config=None, **kwargs):
        ...
        #
        broker_int_broker_config_dict = {
            broker_int: self.get_resource_config_dict(
                ResourceType.BROKER, str(broker_int)
            )
            for broker_int in broker_dict
        }
        #
    ...
    def get_resource_config_dict(self, resourceType, resource_str):
        ...
        configEntry_dict = self.adminClient.describe_configs([configResource])[
            configResource
        ].result()
        # config_dict: str -> str
        config_dict = {
    ...
    def set_resource_config_dict(
        self, resourceType, resource_str, new_config_dict, **kwargs
    ):
        test_bool = kwargs["test"] if "test" in kwargs else False
        #
        old_config_dict = self.get_resource_config_dict(
        ...
        #
        alter_configResource = ConfigResource(
            resourceType, resource_str, set_config=alter_config_dict
        )
        #
        future = self.adminClient.alter_configs(
            [alter_configResource], validate_only=test_bool
        )[alter_configResource]
        #
        future.result()
    ...
    def delete_groups(self, pattern, state_pattern="*"):
        ...
        if not group_str_list:
            return []
        #
        group_str_group_future_dict = self.adminClient.delete_consumer_groups(
            group_str_list
        )
    ...
    def describe_groups(self, pattern="*", state_pattern="*"):
        ...
        if not group_str_list:
            return {}
        #
        group_str_consumerGroupDescription_future_dict = (
            self.adminClient.describe_consumer_groups(group_str_list)
        )
        group_str_group_description_dict_dict = {
    ...
    def groups(self, pattern="*", state_pattern="*", state=False):
        ...
        #
        listConsumerGroupsResult = self.adminClient.list_consumer_groups(
            states=consumerGroupState_set
        ).result()
        consumerGroupListing_list = listConsumerGroupsResult.valid
        #
        group_str_state_str_dict = {
            consumerGroupListing.group_id: consumerGroupState_to_str(
                consumerGroupListing.state
            )
            for consumerGroupListing in consumerGroupListing_list
            if any(
                fnmatch(consumerGroupListing.group_id, pattern_str)
        ...
        return (
            group_str_state_str_dict
            if state
            else list(group_str_state_str_dict.keys())
        )
    ...
    def group_offsets(self, pattern, group_offsets=None, state_pattern="*"):
        ...
        if group_offsets_dict is not None:
            ...
            consumerGroupTopicPartitions = (
                group_str_group_offsets_to_consumerGroupTopicPartitions(
                    group_str, group_offsets
                )
            )
            group_str_consumerGroupTopicPartitions_future_dict = (
                self.adminClient.alter_consumer_group_offsets(
                    [consumerGroupTopicPartitions]
                )
            )
            #
            group_offsets = group_str_consumerGroupTopicPartitions_future_dict_to_group_offsets(
                group_str_consumerGroupTopicPartitions_future_dict
            )
        else:
            group_str_list = self.groups(pattern, state_pattern)
            ...
            consumerGroupTopicPartitions_list = [
                _ConsumerGroupTopicPartitions(group_str)
                for group_str in group_str_list
            ]
            group_str_consumerGroupTopicPartitions_future_dict = (
                self.adminClient.list_consumer_group_offsets(
                    consumerGroupTopicPartitions_list
                )
            )
            #
    ...
    def block_topic(self, topic, exists=True):
        ...
        while True:
            ...
            if num_retries_int >= self.storage_obj.block_num_retries():
                raise Exception(
                    f"Retried waiting for topic {
                        'deletion' if exists else 'creation'} {
                        self.storage_obj.block_num_retries()} times, stopping."
                )
            #
    ...
    def config(self, pattern, config=None, **kwargs):
        ...
        if config is not None:
            for topic_str in topic_str_list:
                self.set_resource_config_dict(
                    ResourceType.TOPIC, topic_str, config_dict, test=test_bool
                )
        #
        topic_str_config_dict_dict = {
            topic_str: self.get_resource_config_dict(
                ResourceType.TOPIC, topic_str
            )
            for topic_str in topic_str_list
        }
        #
    ...
    def create(self, topic, partitions=1, replication=-1, config={}, **kwargs):
        ...
        #
        config_dict["retention.ms"] = (
            config_dict["retention.ms"]
            if "retention.ms" in config_dict
            else self.storage_obj.retention_ms()
        )
        #
        newTopic = NewTopic(
            topic_str,
            partitions_int,
            replication_factor=replication_int,
            config=config_dict,
        )
    ...
    def offsets_for_times(self, pattern, partitions_timestamps, **kwargs):
        ...
        for topic_str in topic_str_list:
            ...
            if topicPartition_list:
                ...
                topicPartition_list1 = consumer.offsets_for_times(
                    topicPartition_list, timeout=timeout_float
                )
                #
                for topicPartition in topicPartition_list1:
                    offsets_dict[topicPartition.partition] = (
    ...
    def partitions(
        self, pattern=None, partitions=None, verbose=False, **kwargs
    ):
        pattern_str_or_str_list = pattern
        partitions_int = partitions
        ...
        if partitions_int is not None:
            ...
            #
            newPartitions_list = [
                NewPartitions(topic_str, partitions_int)
                for topic_str in topic_str_list
            ]
            topic_str_future_dict = self.adminClient.create_partitions(
                newPartitions_list, validate_only=test_bool
            )
            #
        ...
        if verbose_bool:
            ...
            for topic_str, topic_dict in topic_str_topic_dict_dict.items():
                ...
                topic_str_partition_int_partition_dict_dict_dict[topic_str] = (
                    partitions_dict
                )
            #
            ...
            topic_str_partitions_int_dict = {
                topic_str: len(
                    topic_str_topicMetadata_dict[topic_str].partitions
                )
                for topic_str in topic_str_topicMetadata_dict
                if any(
                    fnmatch(topic_str, pattern_str)
    ...
    def watermarks(self, pattern, **kwargs):
        ...
        for topic_str in topic_str_list:
            ...
            partition_int_offsets_tuple_dict = {
                partition_int: consumer.get_watermark_offsets(
                    TopicPartition(topic_str, partition=partition_int),
                    timeout_float,
                )
    ...
    def delete_records(self, pattern_or_offsets, **kwargs):
        request_timeout_float = (
            kwargs["request_timeout"] if "request_timeout" in kwargs else None
        )
        operation_timeout_float = (
        ...
        if isinstance(pattern_or_offsets, dict):
            ...
            for topic_str in topic_str_list:
                ...
                for partition_int in range(0, partitions_int):
                    topicPartition_list.append(
                        TopicPartition(topic_str, partition_int, OFFSET_END)
                    )
        #
        if (
            request_timeout_float is not None
            and operation_timeout_float is not None
        ):
            self.adminClient.delete_records(
                topicPartition_list,
                request_timeout=request_timeout_float,
                operation_timeout=operation_timeout_float,
            )
        elif (
            request_timeout_float is not None
            and operation_timeout_float is None
        ):
            self.adminClient.delete_records(
                topicPartition_list, request_timeout=request_timeout_float
            )
        elif (
            request_timeout_float is None
            and operation_timeout_float is not None
        ):
            self.adminClient.delete_records(
                topicPartition_list, operation_timeout=operation_timeout_float
            )
        else:
...
def group_str_consumerGroupTopicPartitions_future_dict_to_consumerGroupTopicPartitions_list(
    group_str_consumerGroupTopicPartitions_future_dict,
):
    consumerGroupTopicPartitions_list = [
...
def consumerGroupTopicPartitions_list_to_group_offsets(
    consumerGroupTopicPartitions_list,
):
    ...
    for consumerGroupTopicPartitions in consumerGroupTopicPartitions_list:
        ...
        topic_str_offsets_dict_dict = (
            consumerGroupTopicPartitions_to_topic_offsets(
                consumerGroupTopicPartitions
            )
        )
        group_str_topic_str_offsets_dict_dict_dict[group_str] = (
            topic_str_offsets_dict_dict
        )
    #
...
def consumerGroupTopicPartitions_to_topic_offsets(
    consumerGroupTopicPartitions,
):
    ...
    for topicPartition in consumerGroupTopicPartitions.topic_partitions:
        ...
        partition_int_offset_int_tuple = (
        ...
        if topic_str in topic_str_partition_int_offset_int_tuple_list_dict:
            topic_str_partition_int_offset_int_tuple_list_dict[
                topic_str
            ].append(partition_int_offset_int_tuple)
        else:
            topic_str_partition_int_offset_int_tuple_list_dict[topic_str] = [
...
def group_str_consumerGroupTopicPartitions_future_dict_to_group_offsets(
    group_str_consumerGroupTopicPartitions_future_dict,
):
    consumerGroupTopicPartitions_list = group_str_consumerGroupTopicPartitions_future_dict_to_consumerGroupTopicPartitions_list(
        group_str_consumerGroupTopicPartitions_future_dict
    )
    #
    group_offsets = consumerGroupTopicPartitions_list_to_group_offsets(
        consumerGroupTopicPartitions_list
    )
    #
    return group_offsets
...
def group_str_group_offsets_to_consumerGroupTopicPartitions(
    group_str, group_offsets
):
    topicPartition_list = []
    ...
    #
    consumerGroupTopicPartitions = _ConsumerGroupTopicPartitions(
        group_str, topicPartition_list
    )
    #
    return consumerGroupTopicPartitions


def consumerGroupDescription_to_group_description_dict(
...
all_consumerGroupState_str_list = [
    "unknown",
...
def topicMetadata_to_topic_dict(topicMetadata):
    partitions_dict = {
        partition_int: partitionMetadata_to_partition_dict(partitionMetadata)
        for partition_int, partitionMetadata in topicMetadata.partitions.items()
    }
    topic_dict = {
