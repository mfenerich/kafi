...
class ClusterConsumer(KafkaConsumer):
    def __init__(self, cluster_obj, *topics, **kwargs):
        ...
        if "session.timeout.ms" not in self.consumer_config_dict:
            self.consumer_config_dict["session.timeout.ms"] = (
                cluster_obj.session_timeout_ms()
            )
        if "enable.auto.commit" not in self.consumer_config_dict:
            self.consumer_config_dict["enable.auto.commit"] = (
        ...
        def on_assign(consumer, partitions):
            def set_offset(topicPartition):
                if (
                    ...
                    in self.topic_str_start_offsets_dict_dict
                ):
                    offsets = self.topic_str_start_offsets_dict_dict[
                        topicPartition.topic
                    ]
            ...
            if self.topic_str_start_offsets_dict_dict is not None:
                topicPartition_list = [
                    set_offset(topicPartition) for topicPartition in partitions
                ]
    ...
    def consume_impl(self, **kwargs):
        n_int = (
            kwargs["n"] if "n" in kwargs and kwargs["n"] != ALL_MESSAGES else 1
        )
        #
        message_list = self.consumer.consume(
            n_int, self.storage_obj.consume_timeout()
        )
        #
        deserialized_message_dict_list = [
            {
                "topic": message.topic(),
                ...
                "key": self.deserialize(
                    message.key(),
                    self.topic_str_key_type_str_dict[message.topic()],
                ),
                "value": self.deserialize(
                    message.value(),
                    self.topic_str_value_type_str_dict[message.topic()],
                ),
            }
    ...
    def commit(self, offsets=None, **kwargs):
        asynchronous_bool = (
            kwargs["asynchronous"] if "asynchronous" in kwargs else False
        )
        #
        if offsets is not None:
            ...
            if isinstance(str_or_int, str):
                ...
                offsets_dict = {
                    topic_str: offsets for topic_str in self.topic_str_list
                }
            #
            offsets_topicPartition_list = [
                TopicPartition(topic_str, partition_int, offset_int)
                for topic_str, offsets in offsets_dict.items()
                for partition_int, offset_int in offsets.items()
            ]
            #
            commit_topicPartition_list = self.consumer.commit(
                offsets=offsets_topicPartition_list,
                asynchronous=asynchronous_bool,
            )
            #
            offsets_dict = topicPartition_list_to_offsets_dict(
                commit_topicPartition_list
            )
        else:
    ...
    def offsets(self, **kwargs):
        ...
        committed_topicPartition_list = self.consumer.committed(
            assignment_topicPartition_list, timeout=timeout_float
        )
        #
        offsets_dict = topicPartition_list_to_offsets_dict(
            committed_topicPartition_list
        )
        #
        return offsets_dict
