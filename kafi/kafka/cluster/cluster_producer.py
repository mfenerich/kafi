...
class ClusterProducer(KafkaProducer):
    def __init__(self, cluster_obj, topic, **kwargs):
        ...
        #
        self.on_delivery_function = (
            kwargs["on_delivery"] if "on_delivery" in kwargs else None
        )
        #
        # Producer config
        #
    ...
    def produce(self, value, **kwargs):
        ...
        for (
            ...
            #
            timestamp_int = (
                timestamp[1] if isinstance(timestamp, tuple) else timestamp
            )
            #
            self.producer.produce(
                ...
                on_delivery=self.on_delivery_function,
            )
            self.producer.poll(
                0
