...
from kafi.helpers import delete, get, is_pattern, pattern_match, post
...
class RestProxyAdmin(KafkaAdmin):
    ...
    def acls(
        ...
        (rest_proxy_url_str, auth_str_tuple) = (
            self.restproxy_obj.get_url_str_auth_str_tuple_tuple()
        )
        #
        url_str = (
            f"{rest_proxy_url_str}/v3/clusters/{self.cluster_id_str}/acls"
        )
        ...
        response_dict = get(
            ...
            retries=self.restproxy_obj.requests_num_retries(),
        )
        kafkaAcl_dict_list = response_dict["data"]
        #
        dict_list = [
            kafkaAcl_dict_to_dict(kafkaAcl_dict)
            for kafkaAcl_dict in kafkaAcl_dict_list
        ]
        return dict_list
    ...
    def create_acl(
        ...
        #
        payload_dict = {
            "resource_type": restype.upper(),
            "pattern_type": resource_pattern_type.upper(),
    ...
    def delete_acl(
        ...
        url_str = f"{rest_proxy_url_str}/v3/clusters/{
            self.cluster_id_str}/acls?resource_type={
            restype.upper()}&pattern_type={
            resource_pattern_type.upper()}&operation={
                operation.upper()}&permission={
                    permission_type.upper()}"
        if name is not None:
    ...
    def brokers(self, pattern=None):
        ...
        #
        url_str = (
            f"{rest_proxy_url_str}/v3/clusters/{self.cluster_id_str}/brokers"
        )
        headers_dict = {"Content-Type": "application/json"}
        response_dict = get(
            url_str,
        ...
        #
        broker_dict = {
            kafkaBroker_dict["broker_id"]: kafkaBroker_dict["host"]
            + ":"
            + str(kafkaBroker_dict["port"])
            for kafkaBroker_dict in kafkaBroker_dict_list
            if any(
                fnmatch(str(kafkaBroker_dict["broker_id"]), pattern_str)
    ...
    def broker_config(self, pattern=None, config=None, **kwargs):
        ...
        if config_dict is not None:
            url_str = f"{rest_proxy_url_str}/v3/clusters/{
                self.cluster_id_str}/broker-configs:alter"
            headers_dict = {"Content-Type": "application/json"}
            #
            dict_list = [
                {"name": key_str, "value": value_str}
                for key_str, value_str in config_dict.items()
            ]
        ...
        broker_int_broker_config_dict = {
            broker_int: cluster_config_dict for broker_int in broker_dict
        }
        #
    ...
    def describe_groups(self, pattern="*", state_pattern="*"):
        ...
        state_pattern_str_list = (
            [state_pattern] if isinstance(pattern, str) else state_pattern
        )
        #
        kafkaConsumerGroup_dict_list = self.get_kafkaConsumerGroup_dict_list(
            pattern_str_list
        )
        #
        group_str_group_description_dict_dict = {
            kafkaConsumerGroup_dict["consumer_group_id"]: {
                "group_id": kafkaConsumerGroup_dict["consumer_group_id"],
                "is_simple_consumer_group": kafkaConsumerGroup_dict[
                    "is_simple"
                ],
                "partition_assignor": kafkaConsumerGroup_dict[
                    "partition_assignor"
                ],
                "state": kafkaConsumerGroup_dict["state"].lower(),
            }
            for kafkaConsumerGroup_dict in kafkaConsumerGroup_dict_list
            if any(
                fnmatch(
                    kafkaConsumerGroup_dict["consumer_group_id"], pattern_str
                )
                for pattern_str in pattern_str_list
            )
            and any(
                fnmatch(
                    kafkaConsumerGroup_dict["state"].lower(), state_pattern_str
                )
                for state_pattern_str in state_pattern_str_list
            )
        }
        #
        (rest_proxy_url_str, auth_str_tuple) = (
        ...
        for (
        ...
        ) in group_str_group_description_dict_dict.items():
            url_str = f"{rest_proxy_url_str}/v3/clusters/{
                self.cluster_id_str}/consumer-groups/{group_str}/consumers"
            headers_dict = {"Content-Type": "application/json"}
            response_dict = get(
                url_str,
                headers_dict,
            ...
            dict_list = [
                {
                    "member_id": kafkaConsumer_dict["consumer_id"],
                    "client_id": kafkaConsumer_dict["client_id"],
                    "host": kafkaConsumer_dict["cluster_id"],
                    "group_instance_id": kafkaConsumer_dict["instance_id"],
                }
                for kafkaConsumer_dict in kafkaConsumer_dict_list
            ]
            for dict in dict_list:
                consumer_id_str = dict["member_id"]
                url_str = f"{rest_proxy_url_str}/v3/clusters/{
                    self.cluster_id_str}/consumer-groups/{group_str}/consumers/{consumer_id_str}/assignments"
                headers_dict = {"Content-Type": "application/json"}
                response_dict = get(
                    url_str,
                    headers_dict,
                ...
                #
                dict["topic_partitions"] = [
                    {
                        "error": None,
                        "metadata": None,
                        "offset": None,
                        "partition": kafkaConsumerAssignment_dict[
                            "partition_id"
                        ],
                        "topic": kafkaConsumerAssignment_dict["topic_name"],
                    }
                    for kafkaConsumerAssignment_dict in kafkaConsumerAssignment_dict_list
                ]
            #
    ...
    def groups(self, pattern="*", state_pattern="*", state=False):
        ...
        group_str_state_str_dict = {
            kafkaConsumerGroup_dict[
                "consumer_group_id"
            ]: kafkaConsumerGroup_dict["state"].lower()
            for kafkaConsumerGroup_dict in kafkaConsumerGroup_dict_list
            if any(
                fnmatch(
                    kafkaConsumerGroup_dict["consumer_group_id"], pattern_str
    ...
    def group_offsets(self, pattern, group_offsets=None, state_pattern="*"):
        ...
        group_str_list = [
            ...
            and any(
                fnmatch(
                    kafkaConsumerGroup_dict["state"], state_pattern_str.upper()
                )
                for state_pattern_str in state_pattern_str_list
            )
        ]
        ...
        for group_str in group_str_list:
            url_str = f"{rest_proxy_url_str}/v3/clusters/{
                self.cluster_id_str}/consumer-groups/{group_str}/lags"
            response_dict = get(
                url_str,
                headers_dict,
                auth_str_tuple=auth_str_tuple,
    ...
    def config(self, pattern, config=None, **kwargs):
        ...
        if config_dict is not None:
            for topic_str in topic_str_list:
                url_str = f"{rest_proxy_url_str}/v3/clusters/{
                    self.cluster_id_str}/topics/{topic_str}/configs:alter"
                headers_dict = {"Content-Type": "application/json"}
                #
                key_str_value_str_dict_list = [
                    {"name": config_key_str, "value": config_value_str}
                    for config_key_str, config_value_str in config_dict.items()
                ]
        ...
        #
        def kafkaTopicConfigList_dict_to_config_dict(
            kafkaTopicConfigList_dict,
        ):
            config_dict = {}
            #
        ...
        topic_str_config_dict_dict = {
            topic_str: kafkaTopicConfigList_dict_to_config_dict(
                get(
                    f"{rest_proxy_url_str}/v3/clusters/{
                        self.cluster_id_str}/topics/{topic_str}/configs",
                    None,
                    auth_str_tuple=auth_str_tuple,
                    retries=self.restproxy_obj.requests_num_retries(),
                )
    ...
    def create(self, topic_str, partitions=1, config={}, **kwargs):
        ...
        #
        url_str = (
            f"{rest_proxy_url_str}/v3/clusters/{self.cluster_id_str}/topics"
        )
        headers_dict = {"Content-Type": "application/json"}
        configs_dict_list = [
            {"name": config_key_str, "value": config_value_str}
        ...
        payload_dict = {
            ...
            "partitions_count": partitions_int,
            "configs": configs_dict_list,
        }
        post(
            url_str,
            headers_dict,
    ...
    def delete(self, pattern_str_or_str_list, **kwargs):
        ...
        if topic_str_list:
            for topic_str in topic_str_list:
                url_str = f"{rest_proxy_url_str}/v3/clusters/{
                    self.cluster_id_str}/topics/{topic_str}"
                headers_dict = {"Content-Type": "application/json"}
                delete(
                    url_str,
                    headers_dict,
    ...
    def list_topics(self, pattern=None):
        ...
        topic_str_list = [
            kafkaTopic_dict["topic_name"]
            for kafkaTopic_dict in kafkaTopic_dict_list
        ]
        #
    ...
    def partitions(
        ...
        topic_str_partitions_int_dict = {
            kafkaTopic_dict["topic_name"]: kafkaTopic_dict["partitions_count"]
            for kafkaTopic_dict in kafkaTopic_dict_list
            if any(
                fnmatch(kafkaTopic_dict["topic_name"], pattern_str)
        ...
        if verbose_bool:
            ...
            for topic_str in topic_str_partitions_int_dict.keys():
                ...
                for partition_int in range(
                    topic_str_partitions_int_dict[topic_str]
                ):
                    url_str = f"{rest_proxy_url_str}/v3/clusters/{
                        self.cluster_id_str}/topics/{topic_str}/partitions/{partition_int}/replicas"
                    headers_dict = {"Content-Type": "application/json"}
                    response_dict = get(
                        url_str,
                        headers_dict,
                    ...
                    if partition_int in partition_int_partition_dict_dict:
                        partition_dict = partition_int_partition_dict_dict[
                            partition_int
                        ]
                    else:
                        partition_dict = {}
                    ...
                    #
                    partition_int_partition_dict_dict[partition_int] = (
                        partition_dict
                    )
                #
        ...
        for topic_str, partitions_int in topic_str_partitions_int_dict.items():
            ...
            for partition_int in range(partitions_int):
                ...
                topic_str_partition_int_offsets_tuple_dict_dict[topic_str][
                    ...
                    response_dict["beginning_offset"],
                    response_dict["end_offset"],
                )
        #
    ...
    def get_kafkaConsumerGroup_dict_list(self, pattern_str_list):
        ...
        headers_dict = {"Content-Type": "application/json"}
        #
        if len(pattern_str_list) == 1 and not (
            is_pattern(pattern_str_list[0])
        ):
            url_str = f"{rest_proxy_url_str}/v3/clusters/{
                self.cluster_id_str}/consumer-groups/{
                pattern_str_list[0]}"
            response_dict = get(
                url_str,
                headers_dict,
...
def kafkaAcl_dict_to_dict(kafkaAcl_dict):
    dict = {
        "restype": kafkaAcl_dict["resource_type"],
