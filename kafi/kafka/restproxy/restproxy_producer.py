...
from kafi.helpers import base64_encode, is_base64_encoded, post
    ...
    def produce(self, value, **kwargs):
        ...
        for (
            ...
            if self.value_schema_id_int is not None:
                payload_dict = {
                    "value": {
                        "schema_id": self.value_schema_id_int,
                        "data": value,
                    }
                }
            ...
            if key is not None:
                ...
                if self.key_schema_id_int is not None:
                    payload_dict["key"] = {
                        "schema_id": self.key_schema_id_int,
                        "data": key,
                    }
                elif self.key_schema_str is not None:
                    payload_dict["key"] = {
                        "type": type_str,
                        "schema": self.key_schema_str,
                        "data": key,
                    }
                else:
            ...
            if self.keep_timestamps_bool or timestamp is not None:
                if isinstance(timestamp, tuple):
                    timestamp_str = datetime.datetime.fromtimestamp(
                        timestamp[1] / 1000.0, datetime.timezone.utc
            ...
            if headers_str_bytes_tuple_list is not None:
                payload_dict["headers"] = [
                    {
                        "name": headers_str_bytes_tuple[0],
                        "value": base64_encode(
                            headers_str_bytes_tuple[1]
                        ).decode("utf-8"),
                    }
                    for headers_str_bytes_tuple in headers_str_bytes_tuple_list
                ]
            #
            if (
                partition_int is not None
                and partition_int != RD_KAFKA_PARTITION_UA
            ):
                payload_dict["partition_id"] = partition_int
            #
            payload_dict_list.append(
                bytes(json.dumps(payload_dict, default=str), "utf-8")
            )
