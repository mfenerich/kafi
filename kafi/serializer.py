...
class Serializer:
    def serialize(self, payload, key_bool, normalize_schemas=False):
        ...
        schema_id_int = (
            self.key_schema_id_int if key_bool else self.value_schema_id_int
        )
        ...
        def get_schema():
            if schema_str is None:
                if schema_id_int is None:
                    raise Exception(
                        "Please provide a schema or schema ID for the "
                        + ("key" if key_bool else "value")
                        + "."
                    )
        ...
        if payload is None:
            ...
            if type_str.lower() in ["bytes", "str", "json"]:
                ...
                generalizedProtocolMessageType = (
                    self.schema_str_to_generalizedProtocolMessageType(
                        schema, self.topic_str, key_bool, normalize_schemas
                    )
                )
                ...
                serialized_payload_bytes = protobufSerializer(
                    protobuf_message,
                    SerializationContext(self.topic_str, messageField),
                )
            elif type_str.lower() == "avro":
                schema = get_schema()
                avroSerializer = AvroSerializer(
                    self.sr.schemaRegistryClient, schema
                )
                payload_dict = payload_to_payload_dict()
                serialized_payload_bytes = avroSerializer(
                    payload_dict,
                    SerializationContext(self.topic_str, messageField),
                )
                ...
                jSONSerializer = JSONSerializer(
                    schema, self.sr.schemaRegistryClient
                )
                serialized_payload_bytes = jSONSerializer(
                    payload_dict,
                    SerializationContext(self.topic_str, messageField),
                )
            else:
                raise Exception(
                    'Only "bytes", "str", "json", "avro", "protobuf" ("pb") and "jsonschema" ("json_sr") supported.'
                )
        #
    ...
    def schema_str_to_generalizedProtocolMessageType(
        self, schema_str, topic_str, key_bool, normalize_schemas=False
    ):
        schema_hash_int = hash(schema_str)
        if (
            schema_hash_int
            in self.schema_hash_int_generalizedProtocolMessageType_dict
        ):
            generalizedProtocolMessageType = (
            ...
            subject_name_str = self.sr.create_subject_name_str(
                topic_str, key_bool
            )
            schema_dict = self.sr.create_schema_dict(schema_str, "PROTOBUF")
            schema_id_int = self.sr.register_schema(
                subject_name_str, schema_dict, normalize_schemas
            )
            #
