...
class Storage(Shell, Files, AddOns):
    def __init__(
        ...
        #
        self.schema_registry_config_dict = (
            self.config_dict["schema_registry"]
            if "schema_registry" in self.config_dict
            else {}
        )
        #
        self.sr = None
        #
        self.kafi_config_dict = (
            self.config_dict["kafi"]
            if "kafi" in self.config_dict
            else self.config_dict["kash"] if "kash" in self.config_dict else {}
        )
        #
        if "progress.num.messages" not in self.kafi_config_dict:
            self.progress_num_messages(1000)
        else:
            self.progress_num_messages(
                int(self.kafi_config_dict["progress.num.messages"])
            )
        #
        if "consume.batch.size" not in self.kafi_config_dict:
            ...
            self.consume_batch_size(
                int(self.kafi_config_dict["consume.batch.size"])
            )
        #
        if "produce.batch.size" not in self.kafi_config_dict:
            self.produce_batch_size(1000)
        else:
            self.produce_batch_size(
                int(self.kafi_config_dict["produce.batch.size"])
            )
        #
        ...
        if "auto.offset.reset" not in self.kafi_config_dict:
            ...
            self.auto_offset_reset(
                str(self.kafi_config_dict["auto.offset.reset"])
            )
        #
        if "consumer.group.prefix" not in self.kafi_config_dict:
            ...
            self.consumer_group_prefix(
                str(self.kafi_config_dict["consumer.group.prefix"])
            )
        #
        ...
        if "commit.after.processing" not in self.kafi_config_dict:
            ...
            self.commit_after_processing(
                bool(self.kafi_config_dict["commit.after.processing"])
            )
        #
    ...
    def get_config_dict(self, config_str):
        ...
        #
        config_dict = None
        configs_path_str_list = [
            f"{home_str}/configs/{self.dir_str}",
            f"{home_str}/configs",
            f"{home_str}",
        ]
        for configs_path_str in configs_path_str_list:
            if os.path.exists(f"{configs_path_str}/{config_str}.yaml"):
                config_dict = YamlLoader(
                    f"{configs_path_str}/{config_str}.yaml"
                ).load()
            elif os.path.exists(f"{configs_path_str}/{self.config_str}.yml"):
                config_dict = YamlLoader(
                    f"{configs_path_str}/{config_str}.yml"
                ).load()
        if config_dict is None:
            raise Exception(
                f'No configuration file "{config_str}.yaml" or "{config_str}.yml" found in "{configs_path_str_list}" (hint: you can use KAFI_HOME environment variable to set the kafi home directory).'
            )
        #
        for mandatory_section_str in self.mandatory_section_str_list:
            if mandatory_section_str not in config_dict:
                raise Exception(
                    f'Connection configuration file "{config_str}.yaml" does not include a "{mandatory_section_str}" section.'
                )
        #
    ...
    def configs(self, pattern="*", verbose=False):
        ...
        yaml_config_path_str_list = glob.glob(
            f"{configs_path_str}/{pattern_str}.yaml"
        )
        yml_config_path_str_list = glob.glob(
            f"{configs_path_str}/{pattern_str}.yml"
        )
        #
        yaml_config_str_list = [
            re.search(
                f"{configs_path_str}/(.*)\\.yaml", yaml_config_path_str
            ).group(1)
            for yaml_config_path_str in yaml_config_path_str_list
            if re.search(".*/(.*)\\.yaml", yaml_config_path_str) is not None
        ]
        yml_config_str_list = [
            re.search(".*/(.*)\\.yml", yml_config_path_str).group(1)
            for yml_config_path_str in yml_config_path_str_list
            if re.search(".*/(.*)\\.yml", yml_config_path_str) is not None
        ]
        #
        config_str_list = yaml_config_str_list + yml_config_str_list
        #
        if verbose_bool:
            config_str_config_dict_dict = {
                config_str: self.get_config_dict(config_str)
    ...
    def is_headers_tuple_list(self, headers):
        return (
            ...
            and len(headers) > 0
            and all(
                isinstance(header_tuple, tuple)
                and len(header_tuple) == 2
                and isinstance(header_tuple[0], str)
    ...
    def is_headers_dict(self, headers):
        return (
            ...
            and len(headers) > 0
            and all(
                isinstance(header_key, str) for header_key in headers.keys()
            )
        )
    ...
    def is_headers(self, headers):
        return (
            headers is None
            or self.is_headers_tuple_list(headers)
            or self.is_headers_dict(headers)
        )
    ...
    def headers_to_headers_str_bytes_tuple_list(self, headers):
        if headers is None:
            ...
            headers_str_bytes_tuple_list = [
                (header_tuple[0], bytes_or_str_to_bytes(header_tuple[1]))
                for header_tuple in headers
            ]
            ...
            headers_str_bytes_tuple_list = [
                (
                    header_key_str,
                    bytes_or_str_to_bytes(header_value_str_or_bytes),
                )
                for header_key_str, header_value_str_or_bytes in headers.items()
            ]
        else:
            raise Exception(
                "Type error: Headers must either be a list of tuples of strings and bytes, or a dictionary of strings and bytes."
            )
        #
    ...
    def get_schemaRegistry(self):
        schemaRegistry = SchemaRegistry(
            self.schema_registry_config_dict, self.kafi_config_dict
        )
        #
    ...
    def lags(self, group_pattern, topic_pattern, **kwargs):
        ...
        topic_str_partition_int_offsets_tuple_dict_dict = (
            self.admin.watermarks(topic_pattern, **kwargs)
        )
        #
        group_str_topic_str_lags_dict_dict_dict = {}
        for (
            group_str,
            topic_str_group_offsets_dict_dict,
        ) in group_offsets.items():
            group_str_topic_str_lags_dict_dict_dict[group_str] = {}
            for (
                topic_str,
                group_offsets_dict,
            ) in topic_str_group_offsets_dict_dict.items():
                group_str_topic_str_lags_dict_dict_dict[group_str][
                    topic_str
                ] = {
                    partition_int: topic_str_partition_int_offsets_tuple_dict_dict[
                        topic_str
                    ][
                        partition_int
                    ][
                    ...
                    - group_offset_int
                    for partition_int, group_offset_int in group_offsets_dict.items()
                }
        #
    ...
    def offsets_for_times(self, pattern, partitions_timestamps, **kwargs):
        return self.admin.offsets_for_times(
            pattern, partitions_timestamps, **kwargs
        )
